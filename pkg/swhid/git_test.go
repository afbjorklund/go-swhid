...
func TestGitTree(t *testing.T) {
	...
	assert.Nil(t, err)
	err = os.WriteFile(path+"/dir/file", []byte{}, 0o644)
	assert.Nil(t, err)
	blob, err := worktree.Add("dir/file")
	assert.Nil(t, err)
	...
	assert.Nil(t, err)
	tree, err := repo.Tree(hash.String())
	assert.Nil(t, err)
	dir, err := repo.NewDirectoryFromTree(tree)
	assert.Nil(t, err)
	cnt, err := repo.NewContentFromBlob(blob.String())
	assert.Nil(t, err)
	assert.Equal(t, "swh:1:dir:eaf91466ef91bc0c0ac0d9a773f9df797ef29429", dir.Swhid().String())
